...
import { addFibreSubscription, updateFibreSubscription, removeFibreSubscription } from './store.js';
...
// ---------- Page : Nouvel abonnement Fibre ----------

export function renderNouvelleFibre(root, state, actions) {
  const wrap = document.createElement('div');
  wrap.innerHTML = `
    <div class="page-head-row">
      <div>
        <h1 class="page-title">Nouvel abonnement Fibre</h1>
        <p class="page-sub">Les abonnements Fibre ajoutés aujourd'hui apparaissent dans le tableau ci-dessous.</p>
      </div>
  ...
  const subs = state.subscriptionsFibre.filter((s) => s.dateCreation === today);
  renderSubsTable(wrap.querySelector('#tableHost'), subs, state, actions, { emptyText: "Aucun abonnement Fibre ajouté aujourd'hui pour l'instant. Cliquez sur « + Nouvel abonnement » pour en enregistrer un." });
}

// ---------- Page : Historique des abonnements Fibre ----------

export function renderHistoriqueFibre(root, state, actions) {
  const wrap = document.createElement('div');
  wrap.innerHTML = `
    <h1 class="page-title">Historique des abonnements Fibre</h1>
    <p class="page-sub">Retrace tous les abonnements Fibre enregistrés, filtrables par année, mois, vendeur et point de vente.</p>
  ...
  const years = new Set();
  state.subscriptionsFibre.forEach((s) => {
...
function refreshHistorique(wrap, state, actions) {
  ...
  renderSubsTable(wrap.querySelector('#tableHost'), subs, state, actions, { emptyText: 'Aucun abonnement Fibre pour ces filtres.' });
}
...
function filteredSubs(state) {
  ...
  return state.subscriptionsFibre.filter((s) => {
    if (ui.year !== 'ALL') {
...
function renderSubsTable(host, subs, state, actions, { emptyText }) {
  ...
  table.addEventListener('click', (e) => {
    ...
    if (editId) {
      const rec = state.subscriptionsFibre.find((r) => r.id === editId);
      openSubForm(state, actions, rec);
    } else if (delId) {
      if (confirm('Supprimer cet abonnement Fibre ? Cette action est irréversible.')) {
        actions.commit((s) => removeFibreSubscription(s, delId));
      }
    }
  });
...
function openSubForm(state, actions, rec) {
  ...
  const v = rec || {
    ...
    loginSaisie: '', loginPaiement: '', dateDepotAvantages: todayISO(),
    numeroClient: '', numeroFixe: '', infoClient: '', referenceFacture: '', coutFactureInitiale: '', modePaiement: '',
  };

  openModal(`
    <h2>${isEdit ? 'Modifier l’abonnement Fibre' : 'Nouvel abonnement Fibre'}</h2>
    <form id="subForm">
      <div class="form-grid">
  ...
  `, (modalEl) => {
    ...
    if (isEdit) {
      modalEl.querySelector('#btnDelete').addEventListener('click', () => {
        if (confirm('Supprimer cet abonnement Fibre ?')) {
          actions.commit((s) => removeFibreSubscription(s, rec.id));
          closeModal();
        }
      });
    }
    modalEl.querySelector('#subForm').addEventListener('submit', (e) => {
      ...
      if (isEdit) {
        actions.commit((s) => updateFibreSubscription(s, rec.id, data));
      } else {
        actions.commit((s) => addFibreSubscription(s, data));
      }
      closeModal();
    });
  });
